...
export function IntegritySummaryCard({ breakdown, integrityScore }: IntegritySummaryCardProps) {
  const t = useTranslations('candidate.integritySummary')
  ...
  if (integrity.penal_penalty > 0) {
    penalties.push({ label: t('penal'), value: integrity.penal_penalty, color: 'red' })
  }
  const civilTypeKeys: Record<string, string> = {
    violencia_familiar: 'violenciaFamiliar',
    alimentos: 'alimentos',
    laboral: 'laboral',
    contractual: 'contractual',
  }
  for (const civil of integrity.civil_penalties) {
    const key = civilTypeKeys[civil.type]
    penalties.push({
      label: key ? t(key) : civil.type,
      value: civil.penalty,
      color: civil.type === 'violencia_familiar' ? 'red' : 'amber',
    })
  }
  if (integrity.resignation_penalty > 0) {
    penalties.push({ label: t('resignations'), value: integrity.resignation_penalty, color: 'amber' })
  }
  if (integrity.reinfo_penalty > 0) {
    penalties.push({ label: t('reinfo'), value: integrity.reinfo_penalty, color: 'red' })
  }
        ...
        {!hasPenalties ? (
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center justify-between p-2 bg-[var(--muted)] border-2 border-[var(--border)]">
              <span className="text-xs font-bold text-[var(--foreground)] uppercase">{t('baseScore')}</span>
            ...
            <div className={cn('flex items-center justify-between p-2 border-2', scoreColor.border, scoreColor.bg)}>
              <span className="text-xs font-bold text-[var(--foreground)] uppercase">{t('finalScore')}</span>
