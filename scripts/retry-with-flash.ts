/**
 * Retry with Gemini 2.5 Flash (higher rate limit)
 */
...
async function main() {
  ...
  console.log('Candidates to retry: ' + old.length)
  ...
  if (remaining.length > 0) {
    console.log('\nStill with old model:')
    remaining.forEach(r => console.log('  - ' + r.full_name))
  } else {
    console.log('\nAll candidates now use Gemini!')
  }

  const total = await sql`SELECT COUNT(*) as count FROM candidate_proposals WHERE extraction_model LIKE 'gemini-%'`
  console.log('Total Gemini proposals: ' + total[0].count)
}
