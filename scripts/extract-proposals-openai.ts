/**
 * Extrae propuestas de todos los planes de gobierno usando OpenAI GPT-4
...
function loadEnv(): { DATABASE_URL: string; OPENAI_API_KEY: string } {
  const envPath = path.join(process.cwd(), '.env.local')
  const result = { DATABASE_URL: '', OPENAI_API_KEY: '' }
  ...
  if (fs.existsSync(envPath)) {
    ...
    const apiMatch = content.match(/OPENAI_API_KEY=["']?([^"'\n]+)["']?/)
    if (apiMatch) result.OPENAI_API_KEY = apiMatch[1]
  }
...
async function extractProposalsWithAI(text: string): Promise<ExtractedProposal[]> {
  try {
    // Limit text to ~40k chars to stay within context limits
...
async function main() {
  ...
  console.log('║' + ' EXTRACCIÓN DE PROPUESTAS CON OPENAI GPT-4 '.padStart(52).padEnd(68) + '║')
  console.log('╚' + '═'.repeat(68) + '╝')
  ...
  // Create mapping from plan URL to partido name
  ...
  console.log(`📊 Candidatos con plan de gobierno en BD: ${candidates.length}`)
  ...
  // Track processed plan URLs to avoid duplicates
  const processedUrls = new Set<string>()

  for (const candidate of candidates) {
    stats.processed++
    const planUrl = candidate.plan_gobierno_url as string

    // Skip if already processed this plan
    if (processedUrls.has(planUrl)) {
      console.log(`\n[${stats.processed}/${candidates.length}] ${candidate.full_name} - Plan ya procesado`)
      stats.skipped++
      continue
    }

    processedUrls.add(planUrl)
    ...
    const jnePartido = planUrlToPartido.get(planUrl)
    ...
    // Save proposals for ALL candidates with the same plan URL
    const candidatesWithSamePlan = await sql`
      SELECT id, full_name FROM candidates
      WHERE plan_gobierno_url = ${planUrl}
    `

    for (const c of candidatesWithSamePlan) {
      // Delete existing proposals
      await sql`
        DELETE FROM candidate_proposals WHERE candidate_id = ${c.id}
      `
      ...
      console.log(`  💾 ${saved} propuestas guardadas para ${c.full_name}`)
  ...
  console.log(`Planes extraídos: ${stats.succeeded}`)
  console.log(`Planes ya procesados: ${stats.skipped}`)
