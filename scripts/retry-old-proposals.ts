/**
 * Retry extracting proposals for candidates still with old claude-sonnet extractions
 */
...
async function main() {
  console.log('Retrying candidates with old (claude-sonnet) proposals...\n')

  // Find candidates with old extraction model
  const old = await sql`
    SELECT DISTINCT c.id, c.full_name, c.plan_pdf_local
    FROM candidates c
    JOIN candidate_proposals cp ON c.id = cp.candidate_id
    WHERE c.cargo = 'presidente' AND c.is_active = true
    AND cp.extraction_model LIKE 'claude-%'
  `

  console.log('Candidates with old proposals: ' + old.length)
  old.forEach(c => console.log('  - ' + c.full_name + ' (' + (c.plan_pdf_local || 'NO PLAN') + ')'))
  ...
  for (let i = 0; i < old.length; i++) {
    ...
    if (!c.plan_pdf_local) {
      console.log('\n[' + (i + 1) + '] ' + c.full_name + ' - No plan, deleting old proposals')
    ...
    if (!fs.existsSync(pdfPath)) {
      console.log('\n[' + (i + 1) + '] ' + c.full_name + ' - PDF not found')
      continue
    }

    const pdfBuffer = fs.readFileSync(pdfPath)
    const sizeKB = Math.round(pdfBuffer.length / 1024)
    console.log('\n[' + (i + 1) + '/' + old.length + '] ' + c.full_name + ' (' + sizeKB + 'KB)')
    ...
    try {
      ...
      // Delete old proposals and evaluations
      await sql`
        DELETE FROM proposal_evaluations
        WHERE proposal_id IN (SELECT id FROM candidate_proposals WHERE candidate_id = ${c.id})
      `
      await sql`DELETE FROM candidate_proposals WHERE candidate_id = ${c.id}`
  ...
  console.log('\n\nSucceeded: ' + succeeded + '/' + old.length)
  ...
  const remaining = await sql`
    SELECT DISTINCT c.full_name, cp.extraction_model
    FROM candidates c
    JOIN candidate_proposals cp ON c.id = cp.candidate_id
    WHERE c.cargo = 'presidente' AND c.is_active = true
    AND cp.extraction_model LIKE 'claude-%'
  ...
  console.log('Candidates with proposals: ' + withProps[0].count)
  if (remaining.length > 0) {
    console.log('Still with old model:')
    remaining.forEach(r => console.log('  - ' + r.full_name))
  }
}
